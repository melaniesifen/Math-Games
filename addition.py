...
def dot_picture(frame, nums):
    ...
    for i in range(len(nums)): # default is 2
        num_dots = nums[i]
        dot_str = " ".join("." for j in range(num_dots))
        frame.dots = Label(frame, text=dot_str, font=("Courier", 15), bg=g_color).place(relx=x, rely=0.4, anchor=CENTER)
        x += 0.2
        
def check_ans(frame, nums, ans_text, repeat_num):
    global num_correct
    inp = ans_text.get("1.0",END).strip()
    correct_ans = sum(num for num in nums)
    if not inp.isdigit():
        ...
        if repeat_num == 0:
            ...
            count_dots_instructions_label = Label(frame, text='Count the dots', bg=g_color, fg="Red", font=("Courier", 30)).place(relx=0.5, rely=0.7, anchor=CENTER)
